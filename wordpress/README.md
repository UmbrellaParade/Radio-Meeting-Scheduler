...
## Band Schedules And Venue Settings
...
The band workspace has separate studio rehearsal and live schedule tabs. Rehearsals retain duration and time-slot candidates. Live schedules use date-only candidates and have their own local storage, share URL, copy, and notes so live work cannot overwrite rehearsal data.

The settings tab stores separate studio and livehouse registries, each with a name, website URL, and access/map URL. The band JSON backup includes these registries. Importing a legacy backup without a registry preserves the current registrations. Resetting a schedule also preserves the registries.

Selecting a studio or livehouse copies its name and links into the corresponding schedule. Editing a selected registration updates the local schedule; deleting a registration preserves the current schedule's location and links. Existing shared pages change only when the host explicitly updates them. Venue links are included in the GAS memo and member message; guest pages safely link HTTP(S) addresses without a backend schema change.
