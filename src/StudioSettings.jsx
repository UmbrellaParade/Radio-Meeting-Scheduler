...
export default function StudioSettings({ studios, onSave, onDelete }) {
  ...
  const save = (event) => {
    ...
    if (!draft.name.trim()) { setError("スタジオ名を入力してください。"); return; }
  ...
  return <section className="studio-settings">
    <div className="panel-head"><h2>スタジオ登録</h2><span>{studios.length}件</span></div>
    <form className="studio-form" onSubmit={save} noValidate>
      <label className="field wide"><span>スタジオ名</span><input value={draft.name} onChange={(event) => change("name", event.target.value)} required maxLength={200} /></label>
      <label className="field"><span>スタジオ公式サイトURL（任意）</span><input type="url" inputMode="url" value={draft.url} onChange={(event) => change("url", event.target.value)} placeholder="https://" /></label>
      ...
      <div className="studio-form-actions">
        <button className="secondary" type="submit">{draft.id ? <Check size={16} /> : <Plus size={16} />}{draft.id ? "変更を保存" : "スタジオを登録"}</button>
    ...
    <div className="studio-list">
      {studios.map((studio) => <article className="studio-row" key={studio.id}>
        ...
        <div className="inline-actions">
          ...
          <button className="icon-danger" title={`${studio.name}を削除`} aria-label={`${studio.name}を削除`} onClick={() => {
            if (!confirm(`${studio.name}を登録一覧から削除しますか？設定済みの日程と共有ページの場所は変わりません。`)) return;
            onDelete(studio.id);
      ...
      {!studios.length && <p className="empty">登録済みのスタジオはありません。</p>}
    </div>
  </section>;
}
